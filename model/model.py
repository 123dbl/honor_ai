...
class Decoder(nn.Module):
    ...
    def forward(self, 图向量, 操作, trg_mask):
        position = torch.arange(
            0, 图向量.size(1), dtype=torch.long, device=图向量.device
        )
...
class RESNET_Transformer(nn.Module):
    def __init__(self, trg_vocab, d_model, N, heads, dropout, 图向量尺寸=1000):
        ...
        self.resnet = (
            torchvision.models.resnet18(pretrained=False).eval().requires_grad_(True)
        )
...
def get_model(opt, trg_vocab, model_weights="model_weights"):
    ...
    if opt.load_weights is not None and os.path.isfile(
        opt.load_weights + "/" + model_weights
    ):
...
def get_modelB(opt, trg_vocab):
    ...
    model = RESNET_Transformer(
        trg_vocab, opt.d_model, opt.n_layers, opt.heads, opt.dropout
    )

    if opt.load_weights is not None and os.path.isfile(
        opt.load_weights + "/model_weightsB"
    ):
        print("loading pretrained weights...")
